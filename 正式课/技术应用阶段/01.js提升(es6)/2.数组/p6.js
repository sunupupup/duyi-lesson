...
// 得到所有女生
let ret = students.filter((i) => i.sex === '女');

// 得到所有年龄在25岁以下的女生
ret = students.filter((i) => i.age < 25 && i.sex === '女');

// 得到所有姓陈的学生
ret = students.filter((i) => i.name.startsWith('陈'));

// 得到所有电话号码以8结尾的学生
ret = students.filter((i) => i.tel.endsWith('8'));

// 得到所有学生姓名组成的数组
ret = students.map((i) => i.name);

// 得到所有学生姓名组成的字符串，以逗号分割
ret = students.map((i) => i.name).join(',');

// 把所有学生姓名以p元素包起来，形成一个html片段
ret = students.map((i) => `<p>${i.name}</p>`);
...
// 得到所有学生的年龄的总和
ret = students.reduce((pre, cur) => pre + cur.age, 0);
